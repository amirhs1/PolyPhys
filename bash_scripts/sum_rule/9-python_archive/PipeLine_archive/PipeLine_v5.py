...
class Distributions():
    ...
    direction (str): the direction of interest in the geomtery of interest.
    ...
    def __init__(self, histogram, properties, radius_type, geometry, direction):
        if isinstance(histogram, pd.DataFrame):
            self.histogram = histogram
            self.centers = histogram.index.to_numpy()
        else:
    ...
    def _edges_from_centers(self):
        ...
        self.bin_size = np.around(self.centers[1] - self.centers[0],decimals=2) # bin_size
    ...
    def _find_bounds(self):
        ...
        rightmost_idx = -1 * np.ones(len(rightmost),dtype=int) # Initiate the rigtmost bound with the highest possible bound
        for idx, leftmost_value in enumerate(leftmost):
            for edge_idx, edge in enumerate(self.edges[:-1]):
                if (leftmost_value >= edge) and (leftmost_value < self.edges[edge_idx+1]): # the left (or inner) edge index of the bin is set as the index of the bin by which the leftmost side of the bead intersects.
                    leftmost_idx[idx] = edge_idx 
                if (rightmost[idx] >= edge) and (rightmost[idx] < self.edges[edge_idx+1]): # the left (or inner) edge index of the bin is set as the index of the bin by which the rightmost side of the bead intersects.
    ...
    def _consecutive_vol_shares(self):
        """
        _concentric_vol_shares computes the portion of the volume of a bead (a sphere) in consecutive disk-like bins along the longitudinal direction in a cylindrical geometry.
        ...
        for center_idx, bounds_minmax in enumerate(self.particle_bounds):
            ...
            for edge_idx in range(bounds_minmax[0],bounds_minmax[1]+1,1):
                ...
                if np.abs(left_distance) >= self.r_particle: # the most left bound can be a spherical cap or
                    left_distance = -1 * self.r_particle
                right_distance =  self.edges[edge_idx+1]-self.centers[center_idx] # the most right bound can be a spherical cap or
                if np.abs(right_distance) >= self.r_particle: 
                    right_distance = self.r_particle
    ...
    def _concentric_vol_shares(self):
        ...
        for center_idx, bound_minxax in enumerate(self.particle_bounds):
            ...
            for edge_idx in range(bound_minxax[0]+1,bound_minxax[1]+1,1):
                ...
                self.volume_shares[center_idx][edge_idx]= intersect_vol - intersect_vol_previous # The volume share of the previous bin should be subsracted
    ...
    def _volume_fraction(self, col_name):
        ...
        for center_idx in range(n_centers):
            for vol_share_idx, vol_share in self.volume_shares[center_idx].items():
                phi[vol_share_idx] = phi[vol_share_idx] + (rho[center_idx] * (vol_share))
