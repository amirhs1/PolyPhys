...
LAMMPS_VERSION = 20240829
PHI_CS = np.array([0.3, 0.4])                # Crowder volume fractions
RAND_SEED_PARENT = {                         # Initial seeds for randomness
    5.0: {0.3: 10000, 0.4: 20000},
    10.0: {0.3: 30000, 0.4: 40000}
    }
SIG1 = [5.0, 10.0]                           # Diameter of monomers
SIG2 = 1.0                                   # Diameter of crowders
N1 = 2                                       # Number of monomers
BOX_SIZE_FACTOR = 5.0                        # Box size correction factor
...
def generate_ensemble_scripts():
    """
    Generate ensemble bash scripts which can later be used to generated LAMMPS
    ...
    for sig1 in SIG1:
        ...
        for phi_c in PHI_CS:
            ...
            rand_seeds = np.arange(
                    RAND_SEED_PARENT[sig1][phi_c],
                    RAND_SEED_PARENT[sig1][phi_c] + (len(s_ds) + 1) * 10,
                    10,
                    dtype=int
                )
