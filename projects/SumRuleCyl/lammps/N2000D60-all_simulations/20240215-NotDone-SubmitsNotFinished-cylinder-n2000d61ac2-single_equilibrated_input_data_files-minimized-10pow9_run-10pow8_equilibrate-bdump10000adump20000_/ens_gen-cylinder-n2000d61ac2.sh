...
# N2000epsilon5.0r30.5lz233.5sig2.0nc0dt0.01bdump10000adump20000ens1.bug

#			 N		epsilon1	r		lz		sig2	n_crowd		randseed_group	run_dt 	bug_dump 	all_dump
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		0 			4230			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		29456 		4240			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		44184 		4250			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		58912 		4260			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		66276 		4270			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		73640 		4280			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		81004 		4290			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		88368 		4300			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		95732 		4310			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		103096 		4320			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		110460 		4330			0.005	10000		20000"
P[${#P[@]}]="2000	5.0			30.5	233.5	2.0		117824 		4340			0.005	10000		20000"
...
for i in ${ens}; do # ${NAME} uses the value saved in variable called NAME; it is like the LAMMPS itself.
	for ((j=0; j<${#P[@]}; j++ )); do
		...
		if [ $((${n_crowd}+${N})) -ne ${N} ]; then
			if [ $((${n_crowd}+${N})) -le 35000 ]; then
				cp submit_cpu4_le35000.sh submit.sh && mv submit.sh ${dirname}
			elif [ $((${n_crowd}+${N})) -gt 35000 ] && [ $((${n_crowd}+${N})) -le 60000 ]; then
				cp submit_cpu4_gt35000_le60000.sh submit.sh && mv submit.sh ${dirname}	
			elif [ $((${n_crowd}+${N})) -gt 60000 ] && [ $((${n_crowd}+${N})) -le 100000 ]; then
				cp submit_cpu4_gt60000_le100000.sh submit.sh && mv submit.sh ${dirname}	
			else
				cp submit_cpu8_gt100000_le200000.sh submit.sh && mv submit.sh ${dirname}
			fi
		else
