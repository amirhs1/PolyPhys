...
#		echo "mkdir ${dirname} && cd ${dirname} && mkdir archive && lmp_icc_openmpi < ${dirname}.in  > log.out.${CURRENT_DATE}.${dirname}.txt" >> input.txt
		echo "mkdir ${dirname} && cd ${dirname} && mkdir archive && echo Hello World!  > log.out.${CURRENT_DATE}.${dirname}.txt" >> glost_input_${CURRENT_DATE}.txt
	done
done
