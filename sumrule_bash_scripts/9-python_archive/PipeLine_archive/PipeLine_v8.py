...
class Distributions():
    ...
    def __init__(self, histogram, properties, radius_type, geometry, direction, normalized=True):
...
def distributions_generator(histograms, properties, radius_type, geometry, direction, particle_name, to_file=True,normalized=True):
    """
    ...
    for bunch_name, bunch_histogram in histograms.items(): # a bunch of simulations in a group/ensemble/ensemble-averaged group
        distributions = Distributions(bunch_histogram, properties, radius_type, geometry, direction,normalized=normalized)
