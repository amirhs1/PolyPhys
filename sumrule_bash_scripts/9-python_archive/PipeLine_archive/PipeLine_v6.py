...
class Distributions():
    ...
    def _find_bounds(self):
        ...
        for idx, leftmost_value in enumerate(leftmost):
            for edge_idx in range(len(self.edges[:-1])):
                if (leftmost_value >= self.edges[edge_idx]) and (leftmost_value < self.edges[edge_idx+1]): # the left (or inner) edge index of the bin is set as the index of the bin by which the leftmost side of the bead intersects.
                    leftmost_idx[idx] = edge_idx 
                if (rightmost[idx] >= self.edges[edge_idx]) and (rightmost[idx] < self.edges[edge_idx+1]): # the left (or inner) edge index of the bin is set as the index of the bin by which the rightmost side of the bead intersects.
    ...
    def _consecutive_bounds(self):
        ...
        for idx, leftmost_value in enumerate(leftmost):
            for edge_idx in range(len(self.edges[:-1])):
                ...
                if (rightmost[idx] > self.edges[edge_idx]) and (rightmost[idx] <= self.edges[edge_idx+1]): # the left (or inner) edge index of the bin is set as the index of the bin by which the rightmost side of the bead intersects.
                    rightmost_idx[idx] = edge_idx
                #elif rightmost[idx] == self.edges[edge_idx]:
                 #   rightmost_idx[idx] = edge_idx -1 
                #else:
                 #   pass
        self.particle_bounds = np.column_stack((leftmost_idx,rightmost_idx)) 
    ...
    def _concentric_bounds(self):
        ...
        outermost_idx = -1 * np.ones(len(outermost),dtype=int) # Initiate the rigtmost bound with the highest possible bound
