...
def group(ensembles, property_name, geometry, to_file=True):
    """
    group averages over the property_name in all the simulations of each ensemble in the ensembles.
...
def segments_merger(dict_of_segments, df_name, geometry, to_file=True):
    """
    full_ens creates a full-trajectory csv and dataframe from the segment(s) of a ensemble (i.e. a independent simulation with a particular set of inputs such as N,D,ac).
    
    Inputs:
    dict_of_segments (dict of dataframe): a dictonary in which the keys are the name of ensmbles and the values are the dfs in which columns are segments in each ensmeble. The length of a list is equal to the length of each df.
    df_name (str): name of the group of full ensembles
    geomtery (str): shape of the simulation box
    to_file (bool): whether save to file or not
    
    Return:
    dict_of_ensembles (dict): a dictionary in which keys are ensemble names and values are full-trajectory properties. `For exampl
    ...
    # Averging over ensembles with simailar initial paramters
    full_ensembles = {} 
    for ens_name, segment_dfs in dict_of_segments.items():
        list_of_ens_segments = []
        full_ensemble = np.zeros(len(segment_dfs))
        for seg_name in segment_dfs.columns:
            if ens_name in seg_name:
                full_ensemble += segment_dfs[seg_name].values # added but not average since all of them are part of one trajectory
        full_ensemble = np.array(full_ensemble)
        full_ensemble_df = pd.DataFrame(full_ensemble,columns=[ens_name])
        full_ensemble_df_index = segment_dfs.index.values
        full_ensemble_df.set_index(full_ensemble_df_index,inplace=True)
        if to_file:
            output = ens_name+'-'+df_name+'.csv'
            full_ensemble_df.to_csv(output)
        full_ensembles[ens_name] = full_ensemble_df
    return full_ensembles

def hist_of_segments(hist_tuples, df_name, geometry, splitter='all', dtype=float, to_file=True):
    """
    dict_of_segments gets a list of tuples where each tuple has two or more members (one bin edge + N particle types, so a tuple has N+1 members. For example, in cylidrical sumrule, we have bin_edges, histogram of crowders, and histogram of monomers respectivly). 
    This function combines the hisograms of the segments of an ensemble and returns one single histograms. 
    All the bin_edges are the same, so the bin_edge of the first file is used. the lenght of the file_pairs are the same.

    Input:
    csvs (list of tuples): a sorted liste of tuples
    df_name (str): name of the output file
    geomtery (str): shape of the simulation box
    ...
    Return:
    dict_of_segments (dict of dataframes): a dictionary that has name of a segment as its key and  the df of the histogram of that segments as its value.
    ...
    ens_names = []
    for hist_tuple in hist_tuples:
        cell_attrs = cellAttributes(hist_tuple[0],geometry=geometry, splitter='all',warning=False)
        ens_name = cell_attrs.filename.split('ens')[0]+'ens'+str(cell_attrs.ens) 
        ens_names.append(ens_name)
    df_of_segments = dict.fromkeys(ens_names)
    df_of_bin_edges = dict.fromkeys(ens_names) # Using bin centeres as index of dataframes for histograms.
    for ens_name in df_of_segments.keys():
        list_of_segments = []
        for hist_tuple in hist_tuples: #files are organized based on their name
            cell_attrs = cellAttributes(hist_tuple[0],geometry=geometry, splitter='all',warning=False)
            seg_name = cell_attrs.filename # the name of a segment.
            if ens_name in seg_name: # check whether the segment belongs to the ensmble giben by ens_name
                df = pd.read_csv(hist_tuple[0],names=[seg_name],dtype=dtype) # df of a segment with its name as the column name
                list_of_segments.append(df)
                df_of_bin_edges[ens_name] = np.around(np.loadtxt(hist_tuple[1]),decimals=2) # warning bin edges rounded based on the binsize variable in extract trj functions
        df_of_segments[ens_name] = list_of_segments # each key (ensemble name) has a list of dfs of its segments as its values.
        df_of_segments[ens_name] = pd.concat(df_of_segments[ens_name],axis=1) # concatnate all the segment dfs in a list (dict value ) and reassign it to the dict value.
        bin_centers = np.around((df_of_bin_edges[ens_name][:-1] + df_of_bin_edges[ens_name][1:]) / 2.0, decimals=2) # warning bin centeres rounded based on the bin edges variable in extract trj functions
        df_index = pd.Index(bin_centers, dtype = float) # use bin_centers as index
        df_of_segments[ens_name].set_index(df_index,inplace=True)
        
    if to_file:
        for ens_name, ens_hist in df_of_segments.items():
            outname = ens_name+'-'+df_name+'.csv'
            df_of_segments[ens_name].to_csv(outname)
    return df_of_segments

def hist_segment_merger(csv_files,hist_name,geometry,extentions,hist_name_ens, **kwargs):
    # csvs of segments extracted from csv files and then ordered by name
    segment_csvs = file_reader(csv_files,extensions=extentions) 
    # df from each segments saved in dict with filename as key:
    segments = hist_of_segments(segment_csvs,hist_name,geometry,**kwargs)
    # df from all the segments of one ensemble. 
    merged_segments = segments_merger(segments,hist_name_ens,geometry)
...
def rho_phi_generator_all(ensembles, properties, geometry, direction, dir_name, particle_type, particle_name, to_file=True):
    ...
    for groupname, ens_df in ensembles.items():
        ...
        for ens_name in ens_df.columns:
            ...
            integrands = {'cyl_radial':lambda r: 2 * np.pi * properties[condition].lcyl * r,
