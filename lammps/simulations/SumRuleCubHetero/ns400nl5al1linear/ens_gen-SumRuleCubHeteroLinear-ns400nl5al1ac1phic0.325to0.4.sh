#!/bin/bash
# Generated on date 20240911
# ns lcube ac nc randseed_group dt b_dump a_dump al nl ml
P[${#P[@]}]="400	48	1	549160	61080	0.005	2000	5000	1	5	1"
P[${#P[@]}]="400	48	1	591403	61090	0.005	2000	5000	1	5	1"
P[${#P[@]}]="400	48	1	633646	61100	0.005	2000	5000	1	5	1"
P[${#P[@]}]="400	48	1	675889	61110	0.005	2000	5000	1	5	1"
...
for i in ${ens}; do 
	for ((j=0; j<${#P[@]}; j++ )); do
		...
		then
			...
			elif [ $((n_crowd+N)) -gt 500000 ] && [ $((n_crowd+N)) -le 600000 ]; then
				cp submit_cpu40_gt500000_le600000.sh submit.sh && mv submit.sh "${dirname}"
			else
				cp submit_cpu48_gt600000_le700000.sh submit.sh && mv submit.sh "${dirname}"
			fi
		else
