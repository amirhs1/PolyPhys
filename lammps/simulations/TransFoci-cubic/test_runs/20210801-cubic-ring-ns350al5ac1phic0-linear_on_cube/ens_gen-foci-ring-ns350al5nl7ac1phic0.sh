...
P[${#P[@]}]="350	101	1.0		0	30000 0.002	1000	5000	5.0 7	125"
...
for i in ${ens}; do # ${NAME} uses the value saved in variable called NAME; it is like the LAMMPS itself.
	for ((j=0; j<${#P[@]}; j++ )); do
		...
		dirname=ns${n_small}dl${sig2}nl${n_large}dc${sig3}nc${n_crowd}l${l}dt${dt}bdump${bug_dump}adump${all_dump}ens${i}ring
# The command I should use:
		cp foci-ring-ac_equal-cores_less_8.lmp input.lmp
		...
		cp foci-ring-ns350al5nl7.data ${dirname}
