...
P[${#P[@]}]="700	127	1.0		0	30020 0.002	1000	5000	5.0 7	125"
...
for i in ${ens}; do # ${NAME} uses the value saved in variable called NAME; it is like the LAMMPS itself.
	for ((j=0; j<${#P[@]}; j++ )); do
		...
		dirname=ns${n_small}dl${sig2}nl${n_big}dc${sig3}nc${n_crowd}l${l}dt${dt}bdump${bug_dump}adump${all_dump}ens${i}ring
