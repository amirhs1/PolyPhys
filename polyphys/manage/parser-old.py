...
class SumRuleCubHeteroRingBiDisperse(ParserBase):
    """
    parses a `name` (which can be a filename or filepath based on the value
    of `ispath` argument) to extract information about a project's file
    based on a pattern pre-defined by the `lineage` in the
    ...
    In the geometry 'cubic', these patterns are used to parse a `name`:
        segment: al#nl#ns#ac#ab#pctc#nc#nb#l#dt#bdump#adump#ens#.j#.ring
            One of multiple chunks of a complete simulation or measurement.
        whole: al#nl#ns#ns#ac#ab#pctc#nc#nb#l#dt#bdump#adump#ens#.ring
            A complete simulation or measurement; a collection of 'segments'.
        ensemble_long: al#nl#ns#ac#ab#pctc#nc#nb#l#dt#bdump#adump#.ring
            Long name of an ensemble.
        ensemble: al#nl#ns#ac#ab#pctc#nc#nb#
            A collection of 'wholes' (complete simulations) that differs only
            in their initial conditions (e.g., random number seed).
        space: al#nl#ns#ac#ab#pctc#
            A collection of ensembles with a unique set of all the input
            parameters except number of crowders (nc)
    ...
    These are the attributes with "numpy.nan" values for different lineages:
        ...
        whole: 'j'
        ensemble_long: 'ens', and 'j'
        ensemble: 'ens', 'j', 'l', 'dt', 'bdump', and 'adump'
        space: 'ens' , 'j', 'l', 'dt', 'bdump', 'adump', 'nc', and 'nb'
    ...
    nmon_large: int, np.nan
        number of large monomers. Its associated keyword is 'nl'.
    mmon_large: float, default dmon_large**3
        Mass of a large monomer
    nmon: int, np.nan
        Total number of monomers.
    dcrowd_one: int, np.nan
        Size of crowder type one. Its associated keyword is 'ac'.
    dcrowd_two: int, np.nan
        Size of crowder type two. Its associated keyword is 'ab'.
    ncrowd_one: int, np.nan
        number of type-one crowders. Its associated keyword is 'nc'.
    ncrowd_two: int, np.nan
        number of type-two crowders. Its associated keyword is 'nb'.
    mcrowd_one: float, default dcrowd_one**3
        Mass of a type-one crowder
    mcrowd_two: float, default dcrowd_two**3
        Mass of a type-two crowder
    "pct_one": 
        Percentage of type-one crowder volume fraction from total volume
        fraction. Its associated keyword is  "pctc".
    eps_others: float, default 1
        Unit of the LJ interaction strength.
    lcube: float, np.nan
        Side of the cubic simulation box.
    ...
    def _initiate_attributes(self) -> None:
        ...
        self.phi_m_bulk: float = -1
        self.rho_m_bulk: float = -1
        self.dcrowd_one: float = -1
        self.ncrowd_one: int = -1
        self.mcrowd_one: float = -1
        self.phi_c_bulk_one: float = -1
        self.rho_c_bulk_one: float = -1
        self.pct_one: float = -1
        self.dcrowd_two: float = -1
        self.ncrowd_two: int = -1
        self.mcrowd_two: float = -1
        self.phi_c_bulk_two: float = -1
        self.rho_c_bulk_two: float = -1
        self.pct_two: float = -1
        self.ncrowd: int = -1
        self.rho_c_bulk: float = -1
        self.phi_c_bulk: float = -1
        # system attributes
    ...
    def _parse_lineage_name(self) -> None:
        ...
        str_lineages = re.compile(r"([a-zA-Z\-]+)")
        words = str_lineages.split(self.lineage_name)
        attributes_float = ["dmon_large", "dcrowd_one", "dcrowd_two",
                            "pct_one", "lcube", "dt"]
        for attr_n, attr_kw in self._lineage_attributes[self.lineage].items():
            try:
                attr_value = words[words.index(attr_kw) + 1]
        ...
        self.nmon = self.nmon_large + self.nmon_small
        self.mmon_small = self.dmon_small**3
        self.mmon_large = self.dmon_large**3
        self.mcrowd_one = self.dcrowd_one**3
        self.mcrowd_two = self.dcrowd_two**3 
        self.ncrowd = self.ncrowd_one + self.ncrowd_two
        self.pct_two = 100 - self.pct_one
        warnings.warn("Mass is scaled with sized", UserWarning)
    ...
    def _set_parents(self) -> None:
        ...
        ensemble_name = space_name + "nc" + str(self.ncrowd_one) + \
            "nb" + str(self.ncrowd_two)
        ...
        if self.lineage == "space":
            ...
            self.space = self.lineage_name.split("pctc")[0]
            self.ensemble = self.lineage_name
            self.ensemble_long = "N/A"
            self.whole = "N/A"
            self.segment = "N/A"
    ...
    def _bulk_attributes(self) -> None:
        ...
        vol_crowd_one = np.pi * self.dcrowd_one**3 / 6
        vol_crowd_two = np.pi * self.dcrowd_two**3 / 6
        self.rho_c_bulk_one = self.ncrowd_one / vol_cell
        self.rho_c_bulk_two = self.ncrowd_two / vol_cell
        self.rho_c_bulk = self.rho_c_bulk_one + self.rho_c_bulk_two
        self.phi_c_bulk_one = self.rho_c_bulk_one * vol_crowd_one
        self.phi_c_bulk_two = self.rho_c_bulk_two * vol_crowd_two
        self.phi_c_bulk = self.phi_c_bulk_one + self.phi_c_bulk_two
