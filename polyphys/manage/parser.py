...
class ParserBase(ABC):
    """
    Parses a `name` (filename or filepath) to extract information about a
    project's file based on a pattern pre-defined by `lineage`.

    Parameters
    ----------
    name : str
        The name or path of the file to be parsed.
    lineage : {'segment', 'whole', 'ensemble_long', 'ensemble', 'space'}
        The lineage type for this file.
    lineage_attributes : Dict[str, List[str]]
        A dictionary mapping each lineage to an ordered list of expected
        attributes (attribute short names).

    Attributes
    ----------
    filename : str
        The base name of the file without its path.
    filepath : str
        The complete path to the file, or "N/A" if only a filename is provided.
    lineage : str
        The lineage type specified during initialization.
    lineage_attributes : dict
        Dictionary of lineage to attributes mappings.
    ...
    GENEALOGY : dict of lists
        Dictionary defining the hierarchical relationship for each lineage.
    """
    LINEAGES = ["segment", "whole", "ensemble_long", "ensemble", "space"]
    GENEALOGY: Dict[str, List[str]] = {
    ...
    def __init__(
        ...
        # Determine if name is a filepath or filename
        self._filepath, self._filename = \
            ("N/A", name) if '/' not in name and '\\' not in name else os.path.split(name)

        # Validate lineage
        if lineage not in self.LINEAGES:
            raise ValueError(f"Invalid lineage '{lineage}'. Must be one of {self.LINEAGES}.")
        self._lineage = lineage

        # Validate and set lineage_attributes
        for key in lineage_attributes.keys():
            if key not in self.GENEALOGY[lineage]:
                raise ValueError(
                    f"Lineages in 'lineage_attributes' ({list(lineage_attributes.keys())}) "
                    f"are incompatible with the expected genealogy ({self.GENEALOGY[lineage]}) "
                    f"for the lineage '{lineage}'."
                )
        self._lineage_attributes = lineage_attributes

    def __str__(self) -> str:
        return f"Artifact: Name: '{self._filename}', Lineage: '{self._lineage}'"

    def __repr__(self) -> str:
        return f"Artifact('{self._filename}' with lineage '{self._lineage}')"
    ...
    def filepath(self) -> str:
        """Returns the current filepath or 'N/A' if not a path."""
    ...
    @property
    def lineage_attributes(self) -> Dict[str, List[str]]:
        """Returns the lineage_attributes mapping."""
        return self._lineage_attributes

    @abstractmethod
    def _extract_attribute_from_filename(self) -> None:
        """
        Extracts attribute values from the filename based on lineage-specific
        short-names defined in the child class.
        """

    @abstractmethod
    def _set_lineage_names(self) -> None:
        """
        Sets parent lineage names based on the specified lineage, following
        the genealogy defined in GENEALOGY.
        """


class ParserBase3(ABC):
    """
    ...
    def __init__(
        ...
        self._filepath: str = name
        self._filename: str = name
        self._ispath = ispath
        if self._ispath is True:
            self._filename, _ = os.path.splitext(self._filename)
            self._filename = self.filename.split("/")[-1]
        invalid_keyword(lineage, self.LINEAGES)
        self._lineage = lineage
        for key in lineage_attributes.keys():
            if key not in self.GENEALOGY[lineage]:
                raise ValueError(
                    "Lineages (attribute keys)) in 'attributes'"
                    f" ({lineage_attributes.keys()}) are not compatible with the"
                    "expected upper level lineages"
                    f"({self.GENEALOGY[lineage]}) for the given lineage"
                    f"'{lineage}'"
                    )
        self._lineage_attributes = lineage_attributes
    ...
    def __repr__(self) -> str:
        return (
            f"Artifact('{self._filename}' with lineage '{self._lineage}')"
        )
    ...
    @property
    def lineage_attributes(self) -> Dict[str, Dict[str, str]]:
        """
    ...
    @abstractmethod
    def extract_attribute_from_filename(self) -> None:
        """
...
class ParserBase2(ABC):
    ...
    @property
    def physical_attributes(self) -> Dict[str, List[str]]:
        """
...
class TwoMonDep(ParserBase):
    ...
    These are the attributes with "numpy.nan" values for different lineages:
        ...
        ensemble: 'dt', 'bdump', 'adump', 'ens', and 'j'
        space:  'nc', 'dt', 'bdump', 'adump', 'ens' , and 'j'
    ...
    lcube: float, np.nan
        Length of the simulation box, inferred from 'hl' keyword (half of the
        length of the cylindrical confinement along z axis.
    dt: float, np.nan
    ...
    _lineage_attributes: Dict[str, Dict[str, str]] = {
        ...
        "ensemble": {"nmon": "nm", "dmon": "am", "lcube": "hl", "dcrowd": "ac",
                     "ncrowd": "nc", "d_sur": "sd"},
        "space": {"nmon": "nm", "dmon": "am", "lcube": "hl", "dcrowd": "ac"},
    }
    _physical_attributes: Dict[str, List[str]] = {
        "segment": [
            "mmon",
        ...
        "ensemble": ["mmon", "mcrowd"],
        "space": ["mmon", "mcrowd"],
    }
    _geometry_error = "'TwoMonDep' is used for the 'cylindrical' geometry."
    ...
    def _parse_lineage_name(self) -> None:
        ...
        attributes_float = ["dmon", "lcube", "dcrowd", "dt", "d_sur"]
    ...
    def _set_parents(self) -> None:
        ...
        if self.lineage == "space":
            ...
            self.space = (
                "nm" + str(self.nmon) + "am" + str(self.dmon) +
                "ac" + str(self.dcrowd) + "nc" + str(self.ncrowd)
            )
            self.ensemble = self.space + "sd" + str(self.d_sur)
            warnings.warn(convention_warning, UserWarning)
