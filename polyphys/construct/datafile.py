...
def data_file_generator(template, fnames):
    ...
    firstline = f"LAMMPS data file via PipeLine.data_file_generator, \
        based on LAMMPS (template) data file via write_data, version \
        {today}, timestep = 71000000"
    for fname in fnames:
        output = fname.split('/')[-1].split('.lammpstrj')[0]+'.data'
        cell_attrs = SumRule(fname, 'cylindrical')
