...
for res in N*_res/;do
    ...
    cd "$dir" || exit
        ...
        cd test || exit
            nMol=$(head -n 4 "$dir".j02.bug.restart.lammpstrj | tail -n 1)
            lastStep=$(head -n 2 "${dir}".j02.bug.restart.lammpstrj | tail -n 1)
            ...
            mv "${dir}".j02.bug.lammpstrj "${dir}".j02.bug.incomplete.lammpstrj
            linePerStep=$((nMol + 9))
            ...
            sed -i "1,${linePerStep}d" "${dir}".j02.bug.restart.lammpstrj # linux
            cat xx00 "${dir}".j02.bug.restart.lammpstrj > "${dir}".j02.bug.lammpstrj # cat the restart file to the end of corrected file and copy both to a new file
            echo complete
            rm xx00
