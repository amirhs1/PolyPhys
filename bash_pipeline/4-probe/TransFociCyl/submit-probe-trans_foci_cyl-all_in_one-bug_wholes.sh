...
(cd "${file}" && python probe-trans_foci_cyl-all_in_one-bug_wholes.py > "${file}"-probe-trans_foci_cyl-all_in_one-bug_wholes.txt)
}
...
parallel --will-cite --ungroup  --env _ exe {}-gnuparallel_out-probe-trans_foci_cyl-all_in_one-bug_wholes.txt ::: eps*ring/
